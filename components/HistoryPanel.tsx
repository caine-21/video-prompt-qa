...
function FeedbackBadge({ rating }: { rating: 1 | 2 | 3 }) {
  ...
  return (
    <span style={{ background: cfg.bg, border: "2px solid #000", padding: "1px 6px", fontSize: 11, fontWeight: 700 }}>
      {cfg.label}
    </span>
  );
}
...
export default function HistoryPanel({ entries, onSelect, onClear }: Props) {
  ...
  function timeAgo(timestamp: string): string {
    ...
    if (hrs < 24)  return `${hrs}${t("hist.ago.h")}`;
    return `${Math.floor(hrs / 24)}${t("hist.ago.d")}`;
  }
  ...
  const filtered = entries
    ...
    .sort((a, b) => {
      if (sort === "delta") {
        return (b.deltaScore ?? -Infinity) - (a.deltaScore ?? -Infinity);
      }
      return 0;
    });
  ...
  const evalCountLabel = lang === "zh"
    ? `${entries.length} ${t("hist.evals")}`
    : `${entries.length} ${entries.length !== 1 ? t("hist.evals") : t("hist.eval")}`;

  return (
    <div className="neo-card">
      <div className="neo-bar" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
        <span>{t("hist.title")} — {evalCountLabel}</span>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <button
            onClick={() => setFilter(f => f === "all" ? "negative" : "all")}
            style={{ background: filter === "negative" ? "#FF6B6B" : "transparent", border: "2px solid rgba(255,255,255,0.5)", color: "#fff", fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", padding: "2px 10px", cursor: "pointer", fontFamily: "var(--font-space-grotesk), sans-serif" }}
          >
            {filter === "negative"
              ? `${t("hist.showing")} ${negativeCount} — ${t("hist.all")}`
              : `${t("hist.negative")} (${negativeCount})`}
          </button>

          <button
            onClick={() => setSort(s => s === "recent" ? "delta" : "recent")}
            style={{ background: "transparent", border: "2px solid rgba(255,255,255,0.5)", color: "#fff", fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", padding: "2px 10px", cursor: "pointer", fontFamily: "var(--font-space-grotesk), sans-serif" }}
          >
            {sort === "recent" ? t("hist.sort.recent") : t("hist.sort.delta")}
          </button>
          ...
          <button
            onClick={onClear}
            style={{ background: "transparent", border: "2px solid rgba(255,255,255,0.4)", color: "#fff", fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", padding: "2px 10px", cursor: "pointer", fontFamily: "var(--font-space-grotesk), sans-serif" }}
          >
            {t("hist.clear")}
          </button>
        </div>
            ...
            {filtered.map((entry, i) => {
              ...
              return (
                <button
                  ...
                  style={{ display: "block", textAlign: "left", background: "#fff", border: "none", borderRight: i < filtered.length - 1 ? "3px solid #000" : "none", padding: "16px 20px", width: 220, flexShrink: 0, cursor: "pointer", fontFamily: "var(--font-space-grotesk), sans-serif", transition: "background 100ms ease-linear" }}
                ...
                >
                  ...
                  <p style={{ fontSize: 12, fontWeight: 500, lineHeight: 1.4, margin: "0 0 8px", color: "rgba(0,0,0,0.7)" }}>{truncated}</p>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    ...
                    {entry.deltaScore !== undefined && (
                      <span style={{ fontSize: 11, fontWeight: 700, color: entry.deltaScore > 0 ? "#000" : entry.deltaScore < 0 ? "#FF6B6B" : "rgba(0,0,0,0.4)" }}>
