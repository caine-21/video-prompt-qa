...
function ScoreBar({ score }: { score: number }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
      <div style={{ width: 100, height: 16, border: "3px solid #000", background: "#FFFDF5", position: "relative", overflow: "hidden", flexShrink: 0 }}>
        <div style={{ position: "absolute", top: 0, left: 0, height: "100%", width: `${score * 10}%`, background: bg }} />
      </div>
      <span style={{ fontWeight: 700, fontSize: 13, background: bg, border: "2px solid #000", padding: "1px 8px", minWidth: 44, textAlign: "center", display: "inline-block" }}>
        {score.toFixed(1)}
      </span>
    </div>
  );
...
export default function CompareReport({ result }: Props) {
  ...
  const winnerLabel = winner === "tie" ? t("cmp.report.tie") : winner === "A" ? t("cmp.report.winA") : t("cmp.report.winB");
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="px-6 py-6 flex items-center justify-between flex-wrap gap-4">
          <div>
            <p style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.14em", opacity: 0.6, margin: "0 0 6px" }}>
              {t("cmp.report.result")} {provider.toUpperCase()}
            </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        <div className="neo-card" style={winner === "A" ? { background: "#FF6B6B" } : {}}>
          <div className={winner === "A" ? "neo-bar" : "neo-bar-accent"}>
            {winner === "A" ? t("cmp.report.winnerA") : `${t("cmp.report.labelA")} — ${scoreA}/10`}
          </div>
        ...
        <div className="neo-card" style={winner === "B" ? { background: "#FFD93D" } : {}}>
          <div className={winner === "B" ? "neo-bar" : "neo-bar-secondary"}>
            {winner === "B" ? t("cmp.report.winnerB") : `${t("cmp.report.labelB")} — ${scoreB}/10`}
          </div>
      ...
      <div className="neo-card">
        <div className="neo-bar">{t("cmp.report.why")}</div>
